...
fuzz_target!(|data: &[u8]| {
    ...
    let domain = data.get(..2).map_or(1, |bytes| {
        u16::from_le_bytes([bytes[0], bytes[1]]).saturating_sub(1) % 512 + 1
    });
    ...
    if let Ok(dictionary) = Dictionary::parse(data, limits) {
        let terms: Vec<_> = (0..dictionary.len())
            .map(|index| dictionary.term(TermId(index)).unwrap())
            .collect();
        for (index, term) in terms.iter().enumerate() {
            assert_eq!(dictionary.lookup(term).unwrap(), Some(TermId(index as u32)));
            for (end, _) in term
                .char_indices()
                .chain(std::iter::once((term.len(), '\0')))
            {
                let prefix = &term[..end];
                let matches: Vec<_> = terms
                    ...
                    .collect();
                assert_eq!(
                    dictionary.prefix(prefix, 128).unwrap().collect::<Vec<_>>(),
                    matches
                );
            }
        }
    ...
    if let Ok(manifest) = Manifest::parse(data, 128, 4096) {
        let sources: Vec<_> = (0..manifest.len())
            .map(|index| manifest.source(index).unwrap())
            .collect();
        let len =
            records::encode_manifest(manifest.generation, &sources, &mut output, 128).unwrap();
        assert_eq!(&output[..len], data);
    }
    ...
    for (index, bytes) in data.chunks_exact(2).take(512).enumerate() {
        ...
        if index % 2 == 0 {
            ...
            lref[offset as usize] = true;
        } else {
            right.insert(offset).unwrap();
            rref[offset as usize] = true;
        }
    }
    ...
    for offset in 1..=domain {
        let l = lref[offset as usize];
        let r = rref[offset as usize];
    ...
    for &byte in data.iter().take(512) {
        position += u32::from(byte) + 1;
        values.push(position);
    }
    let len = positions::encode(&values, &mut output, 512).unwrap();
    assert_eq!(
        Positions::parse(&output[..len], 512)
            .unwrap()
            .iter()
            .map(Result::unwrap)
            .collect::<Vec<_>>(),
