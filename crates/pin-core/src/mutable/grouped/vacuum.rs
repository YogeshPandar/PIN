...
pub(super) fn retire<S: PageStore>(store: &mut S, mut removable: impl FnMut(RootTid) -> Result<bool>) -> Result<u64> {
    let meta = load(store, 0, PageKind::Meta)?;
    let Some(snapshot) = meta.grouped_state()?.active else { return Ok(0); };
    ...
    while let Some(current) = entry.filter(|entry| entry.key[0] == 0) {
        ...
        if removed != 0 {
            ...
            for (index, chunk) in bytes.chunks(GROUP_DATA_BYTES).enumerate() {
                ...
                if data.id != snapshot.id || data.kind != GroupPageKind::Liveness || data.key != current.key
                    || data.total != value.len || data.offset as usize != index * GROUP_DATA_BYTES
                ...
                if chunk != data.bytes {
                    let mut page = Page::group_record(old.block(), snapshot.id, GroupPageKind::Liveness,
                        current.key, value.len, data.offset, chunk)?;
            ...
            let mut refs = [&meta; 3];
            for (slot, page) in refs.iter_mut().zip(changes.iter().flatten()) { *slot = page; }
            if used == 0 { return Err(Error::InvalidState); }
...
pub(super) fn references(page: &Page, target: u32) -> Result<bool> {
    if page.kind() == PageKind::Meta {
        ...
        return Ok(state.active.is_some_and(|active| [active.head, active.tail, active.root].contains(&target)
                || active.after.is_some_and(|owner| owner.page == target))
            || state.journal.is_some_and(|journal| [journal.head, journal.tail].contains(&target)));
    }
    if page.kind() != PageKind::Grouped { return Ok(false); }
    match page.group_identity()?.1 {
        GroupPageKind::Leaf | GroupPageKind::Branch => {
            ...
            for index in 0..count {
                ...
                if entry.key[0] >> 16 == u64::from(target) { return Ok(true); }
                if level != 0 {
                    if u32::from_le_bytes(entry.value[..4].try_into().map_err(|_| Error::InvalidState)?) == target { return Ok(true); }
                } else {
                    let value = Value::read(entry)?;
                    if value.blocks.contains(&target) || value.members == target { return Ok(true); }
                }
            }
        }
        _ => {
            if page.group_data()?.key[0] >> 16 == u64::from(target) { return Ok(true); }
        }
    }
    Ok(false)
}
