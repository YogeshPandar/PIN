...
impl<'a> Document<'a> {
    ...
    pub const fn identity(self) -> DocumentRef {
        self.identity
    }
    pub const fn analyzed(self) -> &'a Analyzed {
        self.text
    }
...
impl<'a> ReferenceIndex<'a> {
    ...
    pub fn build(
        ...
        for document in documents {
            if document.identity.relation != relation {
                return Err(Error::ForeignRelation);
            }
            total = total
                .checked_add(document.length() as usize)
                .ok_or(Error::Limit("tokens"))?;
            if total > limits.tokens {
                return Err(Error::Limit("tokens"));
            }
            for token in document.text.tokens() {
                if token.term.len() > limits.term_bytes {
        ...
        if ordered
            .windows(2)
            .any(|pair| pair[0].identity.root == pair[1].identity.root)
        {
        ...
        for (ordinal, document) in ordered.iter().enumerate() {
            for token in document.text.tokens() {
                postings.push(Posting {
                    term: token.term,
                    document: ordinal as u32,
                    position: token.position,
                });
            }
        }
        postings.sort_unstable_by(|left, right| {
            (left.term, left.document, left.position).cmp(&(
                right.term,
                right.document,
                right.position,
            ))
        ...
        let distinct = usize::from(!postings.is_empty())
            + postings
                .windows(2)
                .filter(|pair| pair[0].term != pair[1].term)
                .count();
        if distinct > limits.terms {
            return Err(Error::Limit("dictionary terms"));
        }
        ...
        while begin < postings.len() {
            ...
            while end < postings.len() && postings[end].term == text {
                if postings[end - 1].document != postings[end].document {
                    frequency += 1;
                }
                end += 1;
            }
    ...
    pub const fn relation(&self) -> RelationGeneration {
        self.relation
    }
    ...
    pub fn document_frequency(&self, term: &str) -> u64 {
        self.terms
            ...
            .map_or(0, |index| self.terms[index].frequency)
    }
    ...
    pub(crate) fn document_postings(
        ...
        let count = lower_bound(&postings[begin..], work, |posting| {
    ...
    fn has_position(&self, range: Range<usize>, position: u32, work: &mut Work) -> Result<bool> {
        ...
        Ok(postings
            .get(index)
            .is_some_and(|posting| posting.position == position))
    }
    ...
    pub fn search(&self, query: &Query, limits: SearchLimits) -> Result<Search<'_, 'a>> {
        ...
        let phrase_count = query.nodes.iter().try_fold(0usize, |count, node| {
            let additional = if let Kind::Phrase(terms) = &node.kind {
                terms.len()
            } else {
                0
            };
            count
                ...
                .ok_or(Error::Limit("phrase terms"))
        })?;
            ...
            let bound = match &node.kind {
                ...
                Kind::Prefix(prefix) => {
                    ...
                    expanded = expanded
                        ...
                        .ok_or(Error::Limit("prefix expansion"))?;
                    if expanded > limits.expanded_terms {
                        return Err(Error::Limit("prefix expansion"));
                    }
                    Bound::Prefix(range)
                }
                Kind::Phrase(terms) => {
                    ...
                    for (offset, term) in terms.iter().enumerate() {
                        ...
                        phrase_terms.push(PhraseTerm {
                            ...
                            offset: u32::try_from(offset)
                                .map_err(|_| Error::Limit("phrase positions"))?,
                        });
                    }
    ...
    fn matches(&mut self, document: u32) -> Result<bool> {
        for (slot, node) in self.nodes.iter().enumerate() {
            ...
            self.values[slot] = match node {
                ...
                Bound::Term(Some(term)) => !self
                    .index
                    .document_postings(*term, document, &mut self.work)?
                    .is_empty(),
                Bound::Prefix(range) => {
                    let mut found = false;
                ...
                Bound::Phrase(range) => phrase(
                    self.index,
                    &self.phrase_terms[range.clone()],
                    &mut self.ranges,
                    document,
                    &mut self.work,
                )?,
            ...
            match self.matches(ordinal as u32) {
                ...
                Ok(true) => {
                    return Some(Ok(Candidate {
                        document: self.index.documents[ordinal].identity,
...
fn phrase(
    ...
    for (slot, term) in terms.iter().enumerate() {
        ...
        if ranges[slot].is_empty() {
            return Ok(false);
        }
        if ranges[slot].len() < ranges[anchor].len() {
            anchor = slot;
        }
    }
    ...
    for posting in &index.postings[ranges[anchor].clone()] {
        work.charge(1)?;
        let Some(start) = posting.position.checked_sub(terms[anchor].offset) else {
            continue;
        };
        ...
        for (slot, term) in terms.iter().enumerate() {
            ...
            work.charge(1)?;
            let Some(position) = start.checked_add(term.offset) else {
                matched = false;
                break;
            };
            if !index.has_position(ranges[slot].clone(), position, work)? {
                matched = false;
                break;
            }
        }
