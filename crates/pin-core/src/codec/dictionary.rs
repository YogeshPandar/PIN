...
impl<'a> Dictionary<'a> {
    ...
    pub fn parse(bytes: &'a [u8], limits: DictionaryLimits) -> Result<Self> {
        ...
        if profile == 0 { return Err(Error::new(0, ErrorKind::InvalidValue)); }
        let count = reader.u32()?;
        if count > limits.max_terms { return Err(Error::new(4, ErrorKind::LimitExceeded)); }
        let offset_bytes = usize::try_from(count).ok()
            .and_then(|n| n.checked_add(1)).and_then(|n| n.checked_mul(4))
            .ok_or(Error::new(8, ErrorKind::Overflow))?;
        let offsets = reader.take(offset_bytes)?;
        let terms = reader.take(reader.remaining())?;
        let view = Self { profile, offsets, terms, count };
        ...
        for index in 0..count {
            ...
            if term <= previous { return Err(Error::new(8, ErrorKind::InvalidOrder)); }
            previous = term;
        }
        Ok(view)
    }

    pub const fn profile(self) -> u32 { self.profile }
    pub const fn len(self) -> u32 { self.count }
    pub const fn is_empty(self) -> bool { self.count == 0 }

    fn offset(self, index: u32) -> Result<usize> {
        let offset = usize::try_from(index).ok().and_then(|n| n.checked_mul(4))
            .ok_or(Error::new(8, ErrorKind::Overflow))?;
        let bytes = self.offsets.get(offset..).ok_or(Error::new(8, ErrorKind::Truncated))?;
        usize::try_from(Reader::new(bytes).u32()?).map_err(|_| Error::new(8, ErrorKind::Overflow))
    }

    pub fn term(self, id: TermId) -> Result<&'a str> {
        if id.0 >= self.count { return Err(Error::new(8, ErrorKind::InvalidValue)); }
        let start = self.offset(id.0)?;
        let end = self.offset(id.0 + 1)?;
        let bytes = self.terms.get(start..end).ok_or(Error::new(8, ErrorKind::InvalidValue))?;
    ...
    fn lower_bound(self, term: &str) -> Result<u32> {
        ...
        while low < high {
            ...
            if self.term(TermId(middle))? < term { low = middle + 1; } else { high = middle; }
        }
        Ok(low)
    }
    ...
    // counts the complete prefix interval; exceeding the limit never truncates it.
    pub fn prefix(self, prefix: &str, max_expanded: u32) -> Result<Range<u32>> {
        let begin = self.lower_bound(prefix)?;
        let mut end = begin;
        while end < self.count && self.term(TermId(end))?.starts_with(prefix) {
            if end - begin == max_expanded { return Err(Error::new(0, ErrorKind::LimitExceeded)); }
            end += 1;
        }
        Ok(begin..end)
    }
...
pub fn encode(profile: u32, terms: &[&str], output: &mut [u8], limits: DictionaryLimits) -> Result<usize> {
    if profile == 0 { return Err(Error::new(0, ErrorKind::InvalidValue)); }
    ...
    let mut previous = "";
    for &term in terms {
        if term.is_empty() || term.len() > limits.max_term_bytes { return Err(Error::new(0, ErrorKind::LimitExceeded)); }
        if term <= previous { return Err(Error::new(0, ErrorKind::InvalidOrder)); }
        data_bytes = data_bytes.checked_add(term.len()).ok_or(Error::new(0, ErrorKind::Overflow))?;
        previous = term;
    }
    let data_bytes = u32::try_from(data_bytes).map_err(|_| Error::new(0, ErrorKind::Overflow))?;
    let len = terms.len().checked_add(1).and_then(|n| n.checked_mul(4))
        .and_then(|n| n.checked_add(8)).and_then(|n| n.checked_add(data_bytes as usize))
        .ok_or(Error::new(0, ErrorKind::Overflow))?;
    if len.checked_add(HEADER_BYTES).is_none_or(|total| total > limits.max_bytes) {
    ...
    for &term in terms { writer.put(term.as_bytes())?; }
    Ok(writer.len())
}
