...
pub(crate) fn payload(bytes: &[u8], kind: u8, max_bytes: usize) -> Result<&[u8]> {
    ...
    if reader.take(4)? != b"PIN1" {
        return Err(Error::new(0, ErrorKind::BadMagic));
    }
    if reader.u8()? != kind {
        return Err(Error::new(4, ErrorKind::UnknownTag));
    }
    if reader.u8()? != 0 {
        return Err(Error::new(5, ErrorKind::NonCanonical));
    }
    if reader.u16()? != FORMAT_VERSION {
        return Err(Error::new(6, ErrorKind::UnsupportedVersion));
    }
    if reader.u32()? != 0 {
        return Err(Error::new(8, ErrorKind::UnsupportedFeatures));
    }
...
impl DocumentRecord {
    ...
    pub fn encode(self, output: &mut [u8]) -> Result<usize> {
        ...
        writer.u8(match self.publication {
            Publication::Allocated => 0,
            Publication::FragmentsWritten => 1,
            Publication::Published => 2,
            Publication::Abandoned => 3,
        })?;
    ...
    pub fn parse(bytes: &[u8], layout: HeapLayout) -> Result<Self> {
        ...
        let segment =
            SegmentId::new(reader.u64()?).map_err(|_| Error::new(0, ErrorKind::InvalidValue))?;
        let incarnation =
            Incarnation::new(reader.u64()?).map_err(|_| Error::new(8, ErrorKind::InvalidValue))?;
        let block = reader.u32()?;
        let offset = reader.u16()?;
        let root = RootTid::new(block, offset, layout)
            .map_err(|_| Error::new(16, ErrorKind::InvalidValue))?;
        if reader.u16()? != 0 {
            return Err(Error::new(22, ErrorKind::NonCanonical));
        }
        ...
        let live = match reader.u8()? {
            0 => false,
            1 => true,
            _ => return Err(Error::new(33, ErrorKind::InvalidValue)),
        };
        if reader.u16()? != 0 {
            return Err(Error::new(34, ErrorKind::NonCanonical));
        }
        reader.finish()?;
        let record = Self {
            segment,
...
pub fn decode_value(bytes: &[u8], max_bytes: usize) -> Result<Option<&str>> {
    ...
    let result = match reader.u8()? {
        ...
        1 => {
            let len =
                usize::try_from(reader.u32()?).map_err(|_| Error::new(1, ErrorKind::Overflow))?;
            Some(
                std::str::from_utf8(reader.take(len)?)
                    .map_err(|_| Error::new(5, ErrorKind::InvalidUtf8))?,
            )
        }
...
pub fn encode_value(value: Option<&str>, output: &mut [u8], max_bytes: usize) -> Result<usize> {
    ...
    match value {
        ...
        Some(text) => {
            writer.u8(1)?;
            writer
                .u32(u32::try_from(text.len()).map_err(|_| Error::new(0, ErrorKind::Overflow))?)?;
...
impl<'a> Manifest<'a> {
    pub fn parse(bytes: &'a [u8], max_sources: u32, max_bytes: usize) -> Result<Self> {
        ...
        let generation =
            Generation::new(reader.u64()?).map_err(|_| Error::new(0, ErrorKind::InvalidValue))?;
        ...
        for index in 0..count {
            ...
            if source.id.get() <= previous {
                return Err(Error::new(12, ErrorKind::InvalidOrder));
            }
    ...
    pub fn source(self, index: u32) -> Result<Source> {
        ...
        let id = SegmentId::new(reader.u64()?)
            .map_err(|_| Error::new(offset, ErrorKind::InvalidValue))?;
        let sealed = match reader.u8()? {
            0 => false,
            1 => true,
            _ => return Err(Error::new(offset + 8, ErrorKind::UnknownTag)),
        };
        if reader.take(7)?.iter().any(|&byte| byte != 0) {
            return Err(Error::new(offset + 9, ErrorKind::NonCanonical));
        }
        Ok(Source { id, sealed })
    }
}
...
pub fn encode_manifest(
    ...
    let count =
        u32::try_from(sources.len()).map_err(|_| Error::new(0, ErrorKind::LimitExceeded))?;
    if count > max_sources {
        return Err(Error::new(0, ErrorKind::LimitExceeded));
    }
    let mut previous = 0;
