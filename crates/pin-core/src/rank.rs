...
impl CorpusSummary {
    fn validate(self) -> Result<()> {
        ...
        if self.documents == 0 && self.total_length != 0 {
            return Err(Error::InvalidStatistics);
        }
        if self
            .documents
            .checked_mul(u64::from(u32::MAX))
            .is_some_and(|max| self.total_length > max)
        {
            return Err(Error::InvalidStatistics);
        }
        Ok(())
...
impl<'a> StatsEpoch<'a> {
    ...
    pub fn new(
        id: u64,
        summary: CorpusSummary,
        terms: &'a [TermStatistic<'a>],
        limits: StatisticsLimits,
    ) -> Result<Self> {
        let id = NonZeroU64::new(id).ok_or(Error::InvalidStatistics)?;
        summary.validate()?;
        if terms.len() > limits.terms {
            return Err(Error::Limit("statistics terms"));
        }
        ...
        for term in terms {
            bytes = bytes
                ...
                .ok_or(Error::Limit("statistics bytes"))?;
            if term.term.len() > limits.term_bytes || bytes > limits.total_term_bytes {
                return Err(Error::Limit("statistics bytes"));
            }
            if term.term <= previous || term.documents > summary.documents {
                return Err(Error::InvalidStatistics);
            }
            sum = sum
                .checked_add(term.documents)
                .ok_or(Error::InvalidStatistics)?;
            if sum > summary.total_length {
                return Err(Error::InvalidStatistics);
            }
            previous = term.term;
        }
        Ok(Self {
            id,
            summary,
            terms: Statistics::Supplied(terms),
        })
    }
    ...
    pub fn document_frequency(self, term: &str) -> u64 {
        match self.terms {
            Statistics::Supplied(terms) => terms
                .binary_search_by(|entry| entry.term.cmp(term))
                .map_or(0, |index| terms[index].documents),
            Statistics::Indexed(terms) => terms
                .binary_search_by(|entry| entry.text.cmp(term))
                .map_or(0, |index| terms[index].frequency),
        }
    }
}
...
fn idf(summary: CorpusSummary, frequency: u64, boost: f64) -> Result<f64> {
    ...
    if frequency > summary.total_length {
        return Err(Error::InvalidStatistics);
    }
    let missing = summary
        .documents
...
impl ReferenceIndex<'_> {
    ...
    where
        F: FnMut(DocumentRef) -> Result<bool>,
    {
        if epoch.summary.relation != self.relation() {
            return Err(Error::ForeignRelation);
        }
        if epoch.summary.profile != self.profile() {
            return Err(Error::InvalidProfile);
        }
        let required = request
            .offset
            .checked_add(request.limit)
            .ok_or(Error::Limit("top-k offset"))?;
        let mut search = self.search(query, request.search)?;
        let capacity = required.min(self.documents().len());
        if request.limit == 0 || request.offset >= capacity {
            return Ok(Vec::new());
        }
        let weights = self.weights(&mut search, epoch)?;
        let requested = capacity
            .checked_mul(std::mem::size_of::<HeapEntry>())
            .ok_or(Error::Limit("top-k bytes"))?;
        if requested > search.budget.remaining() {
            return Err(Error::Limit("top-k bytes"));
        }
        ...
        while let Some(candidate) = search.next() {
            ...
            if !eligible(candidate.document)? {
                continue;
            }
            ...
            for weight in &weights {
                search.work.charge(1)?;
                let positions =
                    self.document_postings(weight.term, candidate.ordinal, &mut search.work)?;
                let frequency =
                    u32::try_from(positions.len()).map_err(|_| Error::InvalidStatistics)?;
                score = finite(
                    score
                        + request
                            .bm25
                            .weighted_score(weight.value, frequency, length, average)?,
                )?;
            }
            let entry = HeapEntry {
        ...
        for entry in entries.into_iter().skip(request.offset).take(request.limit) {
            output.push(RankedCandidate {
                document: self.documents()[entry.ordinal as usize].identity(),
                score: entry.score,
            });
        }
        Ok(output)
    ...
    fn weights(&self, search: &mut Search<'_, '_>, epoch: StatsEpoch<'_>) -> Result<Vec<Weight>> {
        ...
        for node in &search.nodes {
            ...
            count = count
                ...
                .ok_or(Error::Limit("scoring terms"))?;
        }
        ...
        for (slot, node) in search.nodes.iter().enumerate().rev() {
            ...
            match node {
                ...
                Bound::And(left, right) | Bound::Or(left, right) => {
                    negative[*left] = polarity;
                    negative[*right] = polarity;
                }
                Bound::Term(Some(term)) if !polarity => weights.push(Weight {
                    term: *term,
                    value: 0.0,
                }),
                Bound::Phrase(range) if !polarity => {
                    for token in &search.phrase_terms[range.clone()] {
                        search.work.charge(1)?;
                        if let Some(term) = token.term {
                            weights.push(Weight { term, value: 0.0 });
                        }
                    }
                }
        ...
        for weight in &mut weights {
            ...
            weight.value = idf(
                epoch.summary,
                epoch.document_frequency(self.terms[weight.term].text),
                1.0,
            )?;
        }
        Ok(weights)
