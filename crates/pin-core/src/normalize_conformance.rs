...
fn nfc(text: &str) -> String {
    ...
    let mut output = |value, _: &mut MemoryBudget| { result.push(value); Ok(()) };
    for value in text.chars() { state.push(value, &mut output, &mut budget).unwrap(); }
...
fn from_hex(text: &str) -> String {
    text.split_whitespace().map(|code| char::from_u32(u32::from_str_radix(code, 16).unwrap()).unwrap()).collect()
}
...
fn unicode16_nfc_conformance() {
    ...
    let text = std::fs::read_to_string(std::path::Path::new(&directory).join("NormalizationTest.txt")).unwrap();
    ...
    for (line, raw) in text.lines().enumerate() {
        ...
        if data.starts_with('@') { part_one = data.starts_with("@Part1"); continue; }
        ...
        for input in &columns[..3] { assert_eq!(nfc(input), columns[1], "line {}", line + 1); }
        for input in &columns[3..] { assert_eq!(nfc(input), columns[3], "line {}", line + 1); }
        let expected: String = columns[0].nfc().map(|value| fold(value).unwrap()).nfc().collect();
        let mut budget = MemoryBudget::new(1 << 20);
        assert_eq!(profile_text(&columns[0], 1 << 20, &mut budget).unwrap().0, expected);
        cases += 1;
    }
    assert!(cases > 19_000);
