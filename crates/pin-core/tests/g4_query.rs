...
fn independent_oracle_matches_are_always_covered() {
    let documents = ["", "a", "b", "c", "a b", "b a", "a a", "b c", "a b c", "alphabet", "É a"];
    let mut store = seeded(&documents);
    let atoms = ["a", "b", "missing", "a*", "\"a b\"", "\"a a\"", "NOT a", "NOT (a OR b)"];
    ...
    for source in sources {
        ...
        for (index, text) in documents.iter().enumerate() {
            ...
            assert!(!exact || candidates.contains(&root(index as u32)), "{source:?} on {text:?}");
        }
    }
}
...
fn positive_boolean_candidates_equal_the_oracle_without_shared_cursor_skips() {
    ...
    let mut store = seeded(&documents);
    for source in [
        "a AND b", "a OR b", "a OR a", "a AND a", "a OR (a AND c)",
        "(a AND c) OR a", "(a OR b) AND (b OR c)", "(a OR b) AND a",
        "(a AND b) OR (a AND c)", "a AND missing", "missing OR b",
    ] {
        let query = Query::parse(source, QueryLimits::default()).unwrap();
        let rows = run(&mut store, source, 1 << 20).unwrap();
        let expected: BTreeSet<_> = documents.iter().enumerate().filter_map(|(index, text)| {
            let document = Analyzed::analyze(text, AnalysisLimits::default()).unwrap();
            oracle::matches(&document, &query, 1 << 20, 1 << 20).unwrap().then_some(root(index as u32))
        }).collect();
        assert_eq!(rows.len(), expected.len(), "{source}");
        assert_eq!(rows.into_iter().collect::<BTreeSet<_>>(), expected, "{source}");
    }
}
...
fn phrases_intersect_terms_but_do_not_claim_position_or_multiplicity_checks() {
    ...
    assert_eq!(run(&mut store, "\"a b\"", 1 << 20).unwrap(), vec![root(1), root(2)]);
    assert_eq!(run(&mut store, "\"a a\"", 1 << 20).unwrap(), vec![root(0), root(1), root(2), root(4)]);
    assert!(run(&mut store, "\"a missing\"", 1 << 20).unwrap().is_empty());
}
...
fn sealed_pages_mutable_tails_and_vacuum_preserve_boolean_results() {
    ...
    for index in 0..3300 {
        mutable::insert(&mut store, root(index), if index % 7 == 0 { &both } else { &common }).unwrap();
    }
    mutable::compact(&mut store).unwrap();
    ...
    mutable::compact(&mut store).unwrap();
    let expected: Vec<_> = expected.into_iter().filter(|root| !removed.contains(root)).collect();
    assert_eq!(run(&mut store, "a AND b", 1 << 20).unwrap(), expected);
}
...
fn cursor_budget_falls_back_before_emission_without_truncating_matches() {
    ...
    assert_eq!(run(&mut store, "a AND b", 256).unwrap(), vec![root(0), root(1)]);
    assert!(run(&mut store, "a AND b", 0).is_err());
    let query = Query::parse("a OR b", QueryLimits::default()).unwrap();
    let mut calls = 0;
    assert_eq!(mutable::scan_query(&mut store, &query, 1 << 20, |_| {
        calls += 1;
        Err(Error::InvalidParameters)
    }), Err(Error::InvalidParameters));
    assert_eq!(calls, 1, "an output error must not replay the scan through fallback");
}

#[test]
fn mutable_owner_reordering_is_rejected() {
    let mut store = seeded(&["a", "a", "a", "a"]);
    let block = (1..store.pages.len()).find(|&block| {
        store.read(block as u32).unwrap().kind() == PageKind::Postings
    }).unwrap();
...
fn equal_owner_coordinates_with_foreign_incarnations_are_rejected() {
    ...
    for block in 1..store.pages.len() {
        ...
        if page.kind() != PageKind::Dictionary { continue; }
...
impl PageStore for CountStore {
    ...
    fn interrupt(&mut self) -> Result<()> {
        if self.cancel { Err(Error::InvalidParameters) } else { Ok(()) }
    }
}
...
fn filtering_avoids_owner_page_reads_for_rejected_postings() {
    ...
    let mut store = CountStore { inner, owner_reads: 0, cancel: false };
    ...
    store.owner_reads = 0;
    assert_eq!(run(&mut store, "a AND b", 1 << 20).unwrap(), vec![root(2000)]);
    assert_eq!(store.owner_reads, 1);
    assert!(baseline_reads > store.owner_reads);
    store.owner_reads = 0;
    assert!(run(&mut store, "a AND missing", 1 << 20).unwrap().is_empty());
    assert_eq!(store.owner_reads, 0);
    store.cancel = true;
    assert_eq!(run(&mut store, "a AND b", 1 << 20), Err(Error::InvalidParameters));
}

#[test]
fn deep_queries_use_bounded_explicit_continuations_not_recursive_calls() {
    let source = (0..700).map(|index| if index % 2 == 0 { "a" } else { "b" }).collect::<Vec<_>>().join(" OR ");
    let query = Query::parse(&source, QueryLimits {
        depth: 1024, nodes: 2048, terms: 1024, memory_bytes: 16 << 20,
        ..QueryLimits::default()
    }).unwrap();
    let mut store = seeded(&["a", "b", "c"]);
    let mut rows = Vec::new();
    mutable::scan_query(&mut store, &query, 16 << 20, |root| { rows.push(root); Ok(()) }).unwrap();
