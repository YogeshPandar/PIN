...
fn check(store: &mut impl PageStore, docs: &BTreeMap<RootTid, &str>, source: &str) {
    ...
    for (&root, &text) in docs {
        ...
        if oracle::matches(&document, &query, 1 << 20, 1 << 20).unwrap() { expected.insert(root); }
    }
    let mut actual = BTreeSet::new();
    for (root, recheck) in rows {
        if !docs.contains_key(&root) { continue; }
        assert!(recheck || expected.contains(&root), "false proof: {source}, {root:?}");
        if !recheck || expected.contains(&root) { actual.insert(root); }
    }
...
fn free_pages(store: &mut MemoryStore) -> BTreeSet<u32> {
    ...
    for block in 1..store.blocks().unwrap() {
        assert_eq!(result.contains(&block), store.read(block).unwrap().kind() == PageKind::Free);
    }
    result
}
...
fn complete_generation_boolean_matches_equal_the_independent_owner_oracle() {
    ...
    for (index, text) in ["", "a", "b", "c", "a b", "b a", "a c", "a b c", "alphabet"].iter().enumerate() {
    ...
    for left in atoms {
        ...
        for right in atoms {
            for op in ["AND", "OR"] {
                ...
                assert!(raw(&mut store, &query, 8 << 20).unwrap().iter().all(|(_, recheck)| !recheck));
...
fn new_writes_are_searchable_before_another_snapshot() {
    ...
    for (&root, &text) in &docs { insert(&mut store, root, text); }
    ...
    assert!(raw(&mut store, "newterm", 8 << 20).unwrap().contains(&(root(1, 2), true)));
    build(&mut store).unwrap();
    assert_eq!(raw(&mut store, "a AND b", 8 << 20).unwrap(), vec![(root(1, 2), false)]);
...
fn vacuum_clears_old_incarnations_before_slot_reuse() {
    ...
    assert!(!raw(&mut store, "a AND b", 8 << 20).unwrap().contains(&(root(1, 1), false)));
    build(&mut store).unwrap();
    assert_eq!(raw(&mut store, "a AND b", 8 << 20).unwrap(), vec![(root(2, 1), false)]);
    assert_eq!(mutable::vacuum(&mut store, |_| Ok(false)).unwrap().removed_documents, 0);
    free_pages(&mut store);
}
...
fn multi_leaf_catalog_iteration_and_seek_keep_group_boundaries() {
    ...
    build(&mut store).unwrap();
    for query in ["a", "b", "a AND b", "a AND NOT b", "NOT missing", "missing OR b"] {
        check(&mut store, &docs, query);
    }
}
...
fn multi_page_liveness_keeps_directory_widths_and_retirement_is_idempotent() {
    let mut store = MemoryStore::default();
    mutable::initialize(&mut store).unwrap();
    for block in 0..256 { insert(&mut store, root(block, 291), "wide"); }
    build(&mut store).unwrap();
    let before: Vec<_> = (0..store.pages.len()).filter_map(|block| {
        let page = store.read(block as u32).ok()?;
        (page.kind() == PageKind::Grouped && page.group_identity().ok()?.1 == GroupPageKind::Liveness)
            .then_some((block, page.bytes().len()))
    }).collect();
    assert_eq!(before.len(), 2);
    mutable::vacuum(&mut store, |_| Ok(true)).unwrap();
    assert!(raw(&mut store, "wide OR NOT wide", 8 << 20).unwrap().is_empty());
    for (block, len) in before { assert_eq!(store.read(block as u32).unwrap().bytes().len(), len); }
    mutable::vacuum(&mut store, |_| Ok(true)).unwrap();
}
...
impl PageStore for FaultStore {
    ...
    fn commit(&mut self, pages: &[&Page]) -> Result<()> {
        ...
        self.commits += 1;
        if self.fail == Some((index, false)) { return Err(Error::InvalidState); }
        self.inner.commit(pages)?;
        if self.fail == Some((index, true)) { return Err(Error::InvalidState); }
        Ok(())
    }
    fn remove_owners(&mut self, page: &Page) -> Result<()> { self.commit(&[page]) }
    fn event(&mut self, stage: Stage) -> Result<()> { self.inner.event(stage) }
}
...
fn every_atomic_publication_and_retirement_boundary_is_restartable() {
    ...
    for (tid, text) in [(root(1, 1), "a"), (root(257, 1), "b"), (root(512, 291), "a b")] {
        insert(&mut inner, tid, text);
    }
    build(&mut inner).unwrap();
    insert(&mut inner, root(1, 2), "a b");
    let docs = BTreeMap::from([(root(1, 1), "a"), (root(257, 1), "b"), (root(512, 291), "a b"), (root(1, 2), "a b")]);
    let baseline = FaultStore { inner, commits: 0, fail: None };
    ...
    for point in 0..complete.commits {
        for after in [false, true] {
            ...
            assert!(build(&mut store).is_err(), "missing boundary {point}:{after}");
            store.fail = None;
            check(&mut store, &docs, "a AND b");
...
fn legacy_metadata_and_small_budget_fallback_remain_readable() {
    ...
    assert_eq!(raw(&mut store, "a AND b", 8 << 20).unwrap(), vec![(root(1, 1), false)]);
    build(&mut store).unwrap();
    assert_eq!(store.read(0).unwrap().bytes().len(), 4232);
    assert_eq!(raw(&mut store, "a", 1 << 16).unwrap(), vec![(root(1, 1), false)]);
}
