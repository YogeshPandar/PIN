...
use pin_core::rank::{
    Bm25, CorpusSummary, RankRequest, StatisticsLimits, StatsEpoch, TermStatistic,
};
        ...
        .map(|(i, text)| {
            Document::new(
                ...
                DocumentRecord {
                    ...
                    incarnation: Incarnation::new(i as u64 + 1).unwrap(),
...
fn summary(n: u64, length: u64) -> CorpusSummary {
    CorpusSummary {
        relation: relation(),
        profile: PROFILE_ID,
        documents: n,
...
fn exhaustive_scores_and_topk_match_independent_formula_and_sort() {
    ...
    for _ in 0..127 {
        ...
        for _ in 0..next() % 32 {
            text.push_str(terms[next() as usize % terms.len()]);
            text.push(' ');
        }
    ...
    for (ordinal, doc) in docs.iter().enumerate() {
        if ordinal % 3 == 0 || !oracle::matches(doc.analyzed(), &query, 1 << 20, 1_000_000).unwrap()
        {
            continue;
        }
        let mut score = 0.0;
        for term in terms {
            let df = texts
                .iter()
                .filter(|text| text.tokens().any(|token| token.term == term))
                .count() as f64;
            let tf = doc
                .analyzed()
                .tokens()
                .filter(|token| token.term == term)
                .count() as f64;
            if tf != 0.0 {
                let idf = (1.0 + (texts.len() as f64 - df + 0.5) / (df + 0.5)).ln();
                score +=
                    idf * tf * 2.2 / (tf + 1.2 * (0.25 + 0.75 * f64::from(doc.length()) / average));
            }
        }
    ...
    for k in [0, 1, 2, 10, 40, 127, usize::MAX] {
        for offset in [0, 3, 19, 127] {
            if k.checked_add(offset).is_none() {
                continue;
            }
            ...
            let actual = index
                .rank(&query, epoch, request, |id| Ok(id.root.block() % 3 != 0))
                .unwrap();
...
fn eligibility_precedes_heap_admission_and_ties_are_root_ordered() {
    ...
    assert_eq!(
        result
            .iter()
            .map(|hit| hit.document.root.block())
            .collect::<Vec<_>>(),
        [0, 1]
    );
    assert!(result.iter().all(|hit| hit.score == 0.0));
    assert!(matches!(
        index.rank(&query("a"), epoch, RankRequest::default(), |_| Err(
...
fn duplicate_terms_phrases_prefixes_and_negation_have_explicit_scoring() {
    ...
    let rank = |source| {
        index
            .rank(&query(source), epoch, RankRequest::default(), |_| Ok(true))
            .unwrap()
    };
    ...
    assert_eq!(
        ...
        rank("a")
            .iter()
            .find(|hit| hit.document == docs[0].identity())
            .unwrap()
            .score
    );
...
fn epoch_coherence_foreign_generations_and_budget_excess_are_rejected() {
    ...
    for terms in &invalid {
        assert!(StatsEpoch::new(1, summary(2, 3), terms, StatisticsLimits::default()).is_err());
    }
    ...
    assert!(
        StatsEpoch::new(
            ...
            &[],
            StatisticsLimits::default()
        )
        .is_err()
    );
    let texts = [analyzed("a")];
    let docs = documents(&texts);
    let index = ReferenceIndex::build(relation(), &docs, IndexLimits::default()).unwrap();
    ...
    assert!(
        (index
            .rank(&query("a"), epoch, RankRequest::default(), |_| Ok(true))
            .unwrap()[0]
            .score
            - std::f64::consts::LN_2)
            .abs()
    ...
    .unwrap();
    assert!(matches!(
        index.rank(&query("a"), wrong, RankRequest::default(), |_| Ok(true)),
        Err(Error::ForeignRelation)
    ));
    assert!(
        index
            .rank(
...
fn finite_kernel_inputs_and_extreme_arithmetic_fail_closed() {
    ...
    for boost in [f64::NAN, f64::INFINITY, -1.0] {
        assert!(bm25.term_score(summary(1, 1), 1, 1, 1, boost).is_err());
    }
    ...
    assert!(
        Bm25::new(f64::MAX, 1.0)
            .unwrap()
            .term_score(summary(1, 2), 1, 2, 2, 1.0)
            .is_err()
    );
    assert!(
        bm25.term_score(
            summary(u64::MAX, u64::MAX),
            u64::MAX,
            u32::MAX,
            u32::MAX,
            1.0
