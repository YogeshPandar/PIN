...
fn compare(texts: &[Analyzed], sources: &[String]) {
    let mut documents: Vec<_> = texts
        .iter()
        .enumerate()
        .map(|(i, text)| Document::new(relation(), record(i, text), text).unwrap())
        .collect();
    documents.reverse();
    ...
    for source in sources {
        ...
        let expected: Vec<_> = index
            .documents()
            .iter()
            .filter(|doc| oracle::matches(doc.analyzed(), &query, 1 << 20, 10_000_000).unwrap())
            .map(|doc| doc.identity())
            .collect();
        let actual: Vec<_> = index
            .search(&query, SearchLimits::default())
            .unwrap()
            .map(|candidate| candidate.unwrap().document)
            .collect();
        assert_eq!(actual, expected, "{source}");
        assert_eq!(
            index
                .candidate_count(&query, SearchLimits::default())
                .unwrap(),
            expected.len() as u64
        );
        let mut wire = [0; 4096];
        ...
        assert_eq!(
            index
                .candidate_count(&restored, SearchLimits::default())
                .unwrap(),
            expected.len() as u64
        );
    }
...
fn seeded_corpora_cover_boolean_phrase_prefix_and_root_ordering() {
    ...
    for _ in 0..193 {
        ...
        for _ in 0..next() % 24 {
            text.push_str(vocabulary[next() as usize % vocabulary.len()]);
            text.push(' ');
        }
...
fn publication_profile_and_incarnation_invariants_are_enforced() {
    ...
    .unwrap();
    assert!(matches!(
        ReferenceIndex::build(relation(), &[first, reused], IndexLimits::default()),
    ...
    assert!(matches!(
        ReferenceIndex::build(relation(), &[other], IndexLimits::default()),
...
fn resource_errors_are_explicit_and_search_errors_are_fused() {
    ...
    for work in 1..100 {
        if let Ok(mut search) = index.search(
            &query("a"),
            SearchLimits {
                work_steps: work,
                ..SearchLimits::default()
...
fn repeated_phrase_terms_are_verified_without_materializing_positions() {
    let texts = [
        analyze(&format!("{}b a a", "a ".repeat(4096))),
        analyze("a b a"),
    ];
    compare(
        &texts,
        &["\"b a a\"".into(), "\"a b a\"".into(), "\"a a a a\"".into()],
    );
    ...
    assert_eq!(
        index.total_length(),
        texts.iter().map(|text| u64::from(text.len())).sum()
    );
}
