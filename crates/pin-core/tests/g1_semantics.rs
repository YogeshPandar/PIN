...
fn unicode_profile_has_frozen_normalization_and_simple_folding() {
    ...
    assert_eq!(
        terms,
        ["café", "café", "σ", "σ", "σ", "ß", "ß", "strasse", "straße"]
    );
    assert_eq!(doc.profile(), PROFILE_ID);
    assert_eq!(
        doc.tokens().map(|token| token.position).collect::<Vec<_>>(),
        (0..9).collect::<Vec<_>>()
    );
...
fn phrase_repetition_negation_empty_and_null_are_distinct() {
    ...
    assert_eq!(
        oracle::matches_nullable(None, &parse("NOT a"), 1000, 1000).unwrap(),
        None
    );
...
fn invalid_syntax_and_complexity_fail_explicitly() {
    for input in [
        ...
        assert!(
            Query::parse(input, QueryLimits::default()).is_err(),
            "{input}"
        );
    }
    for (input, limits) in [
...
fn query_wire_roundtrip_revalidates_versions_bounds_and_utf8() {
    ...
    for source in [
        ...
        for cut in 0..len {
            assert!(Query::decode(&bytes[..cut], QueryLimits::default()).is_err());
        }
    ...
    assert_eq!(
        &bytes[..len],
        b"PIN1\x05\0\x01\0\0\0\0\0\x0d\0\0\0\x01\0\0\0\x01\0\0\0\x01\0\0\0a"
    );
