...
fn compare(text: &str) {
    ...
    for (position, token) in actual.tokens().enumerate() {
        assert_eq!(token.position, position as u32);
    }
...
fn unicode16_simple_default_casefold_conformance() {
    ...
    for code in 0..=0x10ffff {
        let Some(source) = char::from_u32(code) else {
            continue;
        };
        let actual = unicode_case_mapping::case_folded(source)
            .map_or(source, |value| char::from_u32(value.get()).unwrap());
        assert_eq!(
            actual,
            *expected.get(&source).unwrap_or(&source),
            "scalar {code:x}"
        );
    }
    eprintln!(
        "unicode16 simple folding: {} mappings; all scalar defaults",
        expected.len()
    );
}
