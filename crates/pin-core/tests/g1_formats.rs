...
fn offsets(domain: u16, seed: u32) -> OffsetSet {
    ...
    for offset in 1..=domain {
        if (u32::from(offset) * 17 + seed) % 11 < seed % 12 { set.insert(offset).unwrap(); }
    }
    set
}
            ...
            for right_seed in 0..12 {
                ...
                for le in ENCODINGS {
                    for re in ENCODINGS {
                        ...
                        assert_eq!(l.intersection(&r).unwrap().iter().collect::<Vec<_>>(), left_ref.intersection(&right_ref).copied().collect::<Vec<_>>());
                        assert_eq!(l.union(&r).unwrap().iter().collect::<Vec<_>>(), left_ref.union(&right_ref).copied().collect::<Vec<_>>());
                        assert_eq!(l.difference(&r).unwrap().iter().collect::<Vec<_>>(), left_ref.difference(&right_ref).copied().collect::<Vec<_>>());
                    }
                }
            }
        }
...
fn offset_golden_tails_and_truncations() {
    ...
    for cut in 0..len { assert!(OffsetSet::parse(&bytes[..cut], layout).is_err()); }
    bytes[len - 1] |= 2;
    assert!(OffsetSet::parse(&bytes[..len], layout).is_err());
    assert!(set.insert(0).is_err());
    assert!(set.insert(10).is_err());
    assert!(set.union(&OffsetSet::new(HeapLayout::new(8).unwrap())).is_err());
    let mut iter = set.iter();
    assert_eq!(iter.next(), Some(1));
    assert_eq!(iter.next(), Some(9));
    for _ in 0..1000 { assert_eq!(iter.next(), None); }
...
fn document_records_preserve_incarnation_and_reject_invalid_states() {
    ...
    let record = DocumentRecord {
        segment: SegmentId::new(1).unwrap(), incarnation: Incarnation::new(2).unwrap(),
        root: RootTid::new(42, 7, layout).unwrap(), token_count: 3, profile: 1,
        publication: Publication::Published, live: true,
    };
    let mut bytes = [0; DocumentRecord::ENCODED_BYTES];
    record.encode(&mut bytes).unwrap();
    assert_eq!(&bytes[..16], b"PIN1\x01\x00\x01\x00\x00\x00\x00\x00\x24\x00\x00\x00");
    ...
    for cut in 0..bytes.len() { assert!(DocumentRecord::parse(&bytes[..cut], layout).is_err()); }
    assert!(DocumentRecord { publication: Publication::Allocated, ..record }.encode(&mut bytes).is_err());
    let replacement = DocumentRecord { incarnation: Incarnation::new(3).unwrap(), ..record };
    assert_ne!(record, replacement);
    for (offset, bad) in [(0, 0), (5, 1), (6, 2), (8, 1), (16, 0), (36, 0), (38, 1), (48, 255), (49, 2), (50, 1)] {
        record.encode(&mut bytes).unwrap();
        bytes[offset] = bad;
        assert!(DocumentRecord::parse(&bytes, layout).is_err(), "offset {offset}");
    }
}
...
fn values_distinguish_null_empty_and_exact_unicode_bytes() {
    ...
    for value in [None, Some(""), Some("a\0b"), Some("café e\u{301} 東京")] {
        ...
        for cut in 0..len { assert!(records::decode_value(&bytes[..cut], 256).is_err()); }
...
fn dictionary_exact_lookup_prefix_bounds_and_untrusted_offsets() {
    let limits = DictionaryLimits { max_bytes: 1024, max_terms: 10, max_term_bytes: 32 };
    ...
    assert_eq!(dict.prefix("z", 0).unwrap(), 4..4);
    ...
    for cut in 0..len { assert!(Dictionary::parse(&bytes[..cut], limits).is_err()); }
    bytes[24] = 1;
...
fn manifest_validates_unique_sorted_sources_and_reserved_bytes() {
    let sources = [Source { id: SegmentId::new(1).unwrap(), sealed: true }, Source { id: SegmentId::new(3).unwrap(), sealed: false }];
    let mut bytes = [0; 256];
    ...
    for (index, source) in sources.iter().enumerate() { assert_eq!(&manifest.source(index as u32).unwrap(), source); }
    assert!(manifest.source(2).is_err());
    for cut in 0..len { assert!(Manifest::parse(&bytes[..cut], 2, 256).is_err()); }
    assert!(Manifest::parse(&bytes[..len], 1, 256).is_err());
    bytes[37] = 1;
    assert!(Manifest::parse(&bytes[..len], 2, 256).is_err());
    assert!(records::encode_manifest(generation, &[sources[0], sources[0]], &mut bytes, 2).is_err());
}

#[test]
fn deterministic_malformed_payloads_never_panic() {
    let layout = HeapLayout::new(291).unwrap();
    let limits = DictionaryLimits { max_bytes: 256, max_terms: 64, max_term_bytes: 32 };
    let mut bytes = [0; 256];
