...
fn varint_golden_boundaries_and_atomic_errors() {
    ...
    for bytes in [&[128, 0][..], &[129, 0], &[255, 255, 255, 255, 16], &[128; 6]] {
...
fn position_golden_preserves_zero_gaps_and_extremes() {
    ...
    assert_eq!(positions.iter().collect::<Result<Vec<_>, _>>().unwrap(), [0, 1, 128, 300]);
    for prefix in 0..len {
    ...
    assert_eq!(Positions::parse(&bytes[..len], 4).unwrap().iter().last().unwrap().unwrap(), u32::MAX);
    assert_eq!(Positions::parse(&[2, 0, 0, 0, 255, 255, 255, 255, 15, 1], 2).unwrap_err().kind, ErrorKind::Overflow);
}

#[test]
fn byte_extents_fail_without_advancing() {
    let mut reader = Reader::new(&[1, 2, 3]);
    assert_eq!(reader.take(usize::MAX).unwrap_err().kind, ErrorKind::Truncated);
    assert_eq!(reader.offset(), 0);
    reader.take(1).unwrap();
    assert_eq!(reader.take(usize::MAX).unwrap_err().kind, ErrorKind::Overflow);
