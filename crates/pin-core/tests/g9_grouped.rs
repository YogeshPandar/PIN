...
use pin_core::grouped::{
    Bitmap, BitmapKind, GroupKey, HEADER_BYTES, Member, Members, Node, PageOffsets,
    QueryScratch, QueryStats, SegmentGroup, encode_bitmap, encode_members, evaluate, retire,
};
...
fn images(key: GroupKey, members: &[Member]) -> (Vec<u8>, Vec<u8>) {
    ...
    let len = Members::open(&owners).unwrap().encode_liveness(&mut live).unwrap();
    live.truncate(len);
    (owners, live)
}
...
fn run(
    ...
    let terms: Vec<_> = terms.iter().map(|bytes| Bitmap::open(bytes)).collect::<Result<_>>()?;
...
fn oracle(program: &[Node], terms: &[BTreeSet<u64>], live: &BTreeSet<u64>) -> BTreeSet<u64> {
    ...
    for node in program {
        values.push(match *node {
            ...
            Node::Difference(left, right) => values[left].difference(&values[right]).copied().collect(),
...
fn nested_boolean_programs_match_independent_incarnation_sets() {
    let programs = [
        ...
        vec![Node::Term(0), Node::Term(1), Node::And(0, 1), Node::Term(2), Node::Or(2, 3)],
    ...
    for domain in [1, 8, 32, 64, 65, 128, 291, 512] {
        for sample in 1..=12u64 {
            ...
            for _ in 0..160 {
                ...
                roots.insert(RootTid::new(key.base() | u32::from(page), offset, key.layout()).unwrap());
            }
            let members: Vec<_> = roots.into_iter().enumerate().map(|(index, root)| Member {
                root,
            ...
            let terms: Vec<_> = sets.iter().map(|set| posting(key,
                members.iter().filter(|m| set.contains(&m.incarnation.get())).map(|m| m.root)
            )).collect();
...
fn reused_tid_cannot_create_a_cross_generation_and_match() {
    ...
    assert!(run(&old_map, &old_live, &old_terms, &query).unwrap().0.is_empty());
    assert!(run(&new_map, &new_live, &new_terms, &query).unwrap().0.is_empty());
    assert!(run(&old_map, &old_live, &[old_terms[0].clone(), new_terms[1].clone()], &query).is_err());
...
fn duplicate_coordinate_is_rejected_before_overwriting_output() {
    ...
    assert_eq!(encode_members(key, &[one, two], &mut output), Err(Error::DuplicateDocument));
...
fn page_pruning_does_not_decode_a_rejected_payload() {
    ...
    assert!(run(&owners, &live, &terms, &[Node::Term(0), Node::Term(1), Node::Or(0, 1)]).is_err());
}

#[test]
fn shared_retirement_is_clear_only_idempotent_and_replayable() {
    let key = key(0, 512, 1);
    let docs = [member(key, 0, 1, 1), member(key, 0, 512, 2), member(key, 255, 64, 3)];
    ...
    for doc in docs {
        ...
        assert!(live.iter().zip(before).all(|(&after, before)| after & !before == 0));
    ...
    assert_eq!(run(&owners, &original, &terms, &[Node::Term(0)]).unwrap().0.len(), 3);
}
...
fn difference_retains_left_offsets_when_both_terms_share_a_page() {
    ...
    assert_eq!(run(&owners, &live, &terms, &query).unwrap().0, BTreeSet::from([1]));
}
...
fn empty_universe_and_missing_terms_never_invent_offsets() {
    ...
    assert_eq!(run(&owners, &live, &terms, &[Node::Term(0), Node::Not(0)]).unwrap().0, BTreeSet::from([1]));
    assert!(run(&owners, &live, &terms, &[Node::Term(0)]).unwrap().0.is_empty());
}
...
fn final_heap_group_and_every_offset_domain_round_trip() {
    for domain in [1, 7, 8, 63, 64, 65, 291, 511, 512] {
        ...
        let (owners, live) = images(key, &docs);
        let terms = [posting(key, docs.iter().map(|m| m.root))];
        assert_eq!(run(&owners, &live, &terms, &[Node::Term(0)]).unwrap().0, BTreeSet::from([1, 2]));
        let mut output = [0xa5; 256];
        let invalid = PageOffsets { page: 255, offsets: [1, 0, 0, 0, 0, 0, 0, 0] };
...
fn malformed_headers_directories_and_truncations_fail_closed() {
    ...
    for (offset, value) in [(0, 0), (4, 2), (6, 9), (7, 1), (12, 1), (16, 0),
        (24, 0), (32, 0), (34, 2), (36, 1), (40, 0), (72, 1), (74, 0), (75, 1)] {
        let mut bad = bytes.clone();
        bad[offset] = value;
        assert!(Bitmap::open(&bad).is_err(), "accepted changed header byte {offset}");
    }
...
fn bad_programs_and_cancellation_do_not_report_success() {
    ...
    let (owners, live) = images(key, &docs);
    let terms = [posting(key, docs.iter().map(|m| m.root))];
    for bad in [vec![], vec![Node::Term(1)], vec![Node::Not(0)], vec![Node::And(0, 0)], vec![Node::All; 65]] {
    ...
    let mut calls = 0;
    let mut emitted = 0;
    let result = evaluate(&segment, &[term], &[Node::Term(0)], &mut QueryScratch::default(), || {
        calls += 1;
        if calls == 3 { Err(Error::Limit("cancelled")) } else { Ok(()) }
    }, |_, _| { emitted += 1; Ok(()) });
    assert_eq!(result, Err(Error::Limit("cancelled")));
    assert_eq!(emitted, 1);
    let result = evaluate(&segment, &[term], &[Node::Term(0)], &mut QueryScratch::default(), || Ok(()), |_, _| Err(Error::InvalidState));
...
fn sealing_checks_incarnations_before_discarding_owner_identity() {
    ...
    let docs = [member(key, 1, 1, 10), member(key, 1, 512, 20), member(key, 255, 65, 30)];
    let (owners, live) = images(key, &docs);
    let map = Members::open(&owners).unwrap();
    let mut output = vec![0xa5; 20_000];
    let stale = Member { root: docs[0].root, incarnation: Incarnation::new(9).unwrap() };
    for invalid in [vec![stale], vec![docs[0], docs[0]], vec![docs[2], docs[0]],
        vec![member(key, 5, 5, 40)]] {
        assert!(map.encode_posting(&invalid, &mut output).is_err());
        assert!(output.iter().all(|&byte| byte == 0xa5));
    }
    let len = map.encode_posting(&[docs[0], docs[2]], &mut output).unwrap();
    output.truncate(len);
    assert_eq!(output, posting(key, [docs[0].root, docs[2].root]));
    assert_eq!(run(&owners, &live, &[output], &[Node::Term(0)]).unwrap().0, BTreeSet::from([10, 30]));
...
fn nested_pruning_avoids_dead_subtrees_and_unreferenced_nodes() {
    ...
    *broken.last_mut().unwrap() = 0;
    let terms = [broken, posting(key, [docs[1].root]), posting(key, [docs[0].root])];
    // the broken term is beneath a page-empty and, while the last term matches.
    let query = [Node::Term(0), Node::Term(1), Node::And(0, 1), Node::Term(2), Node::Or(2, 3)];
    let (found, stats) = run(&owners, &live, &terms, &query).unwrap();
    ...
    assert_eq!(run(&owners, &live, &terms, &query).unwrap().0, BTreeSet::from([1, 2]));
}

#[test]
fn dense_group_directory_and_borrowed_unaligned_records_round_trip() {
    let key = key(0, 512, 1);
    let pages: Vec<_> = (0..=255u8).map(|page| PageOffsets { page, offsets: [u64::MAX; 8] }).collect();
    let mut encoded = vec![0; 17_480];
    assert_eq!(encode_bitmap(key, BitmapKind::Posting, &pages, &mut encoded).unwrap(), 17_480);
    for shift in 0..8 {
...
fn arbitrary_byte_mutations_never_panic_or_escape_the_domain() {
    let key = key(0, 65, 1);
    let docs = [member(key, 0, 1, 1), member(key, 63, 65, 2), member(key, 255, 64, 3)];
    let (owners, live) = images(key, &docs);
